...
!function a(b, c, d) {
    ...
    1: [function(a, b, c) {
        ...
        function h(a) {
            ...
            var b = g(a);
            return c.isArray(a) && -1 === c.indexOf(a, "length") && b.push("length"),
            b
        }
        function i(a, b) {
            return {
        ...
        c.reduce = function(a, b, c) {
            ...
            for (var f = 0, g = a.length; g > f; ++f)
                a.hasOwnProperty(f) && (e ? d = b(d, a[f], f, a) : (d = a[f],
                e = !0));
            return d
        }
        ,
    ...
    4: [function(a, b, c) {
        ...
        function h(a, b, d) {
            ...
            var o = G.keys(b)
              , p = g(o);
            if (a.showHidden && (o = G.getOwnPropertyNames(b)),
            0 === o.length) {
                if (z(b)) {
                    var q = b.name ? ": " + b.name : "";
            ...
            if (v(b) && (r = " " + RegExp.prototype.toString.call(b)),
        ...
        function l(a, b, c, d, e, f) {
            ...
            F(d, e) || (g = "[" + e + "]"),
            i || (G.indexOf(a.seen, j.value) < 0 ? (i = p(c) ? h(a, j.value, null) : h(a, j.value, c - 1),
            i.indexOf("\n") > -1 && (i = f ? i.split("\n").map(function(a) {
                return "  " + a
            }).join("\n").substr(2) : "\n" + i.split("\n").map(function(a) {
                return "   " + a
    ...
    6: [function(a, b, c) {
            ...
            for (var k = 1, l = !1, n = 0, o = a.length; o > n; ++n) {
                ...
                if (a.slice(n, e.length + n) == e) {
                    ...
                    var t = a.indexOf(g, n)
                      ...
                      , w = null
                      , x = 0;
                    if ("-" == u[u.length - 1] && (u = u.substring(0, u.length - 2),
                    l = !0),
        ...
        c.compile = function(a, b) {
            ...
            try {
                ...
                throw "SyntaxError" == l.name && (l.message += b.filename ? " in " + j : " while compiling ejs"),
                l
            }
    ...
    9: [function(a, b) {
        ...
        c.prototype.add = function(a) {
            ...
            for (c in a)
                g.SETTINGS.test(c) && (this._settings[c] = a[c],
                delete a[c]);
            for (f = 0,
            e = i.length; e > f; f++)
                if (i[f].isEqual(a)) {
            ...
            return b || (b = new d(a),
            b.isValid() && (j = this._items.push(b) - 1,
        ...
        c.prototype.discount = function(a) {
            ...
            return b || (b = (parseFloat(this.settings("discount_rate_cart")) || 0) * this.subtotal() / 100),
            a = a || {},
        ...
        c.prototype.remove = function(a) {
            ...
            return 0 === this._items.length && this.destroy(),
            b && (this.save(),
    ...
    10: [function(a, b) {
        ...
        var c = a("./util/mixin")
          , d = b.exports = {
            ...
            target: "",
            duration: 30,
            template: '<%var items = cart.items();var settings = cart.settings();var hasItems = !!items.length;var priceFormat = { format: true, currency: cart.settings("currency_code") };var totalFormat = { format: true, showCode: true };%><form method="post" class="<% if (!hasItems) { %>minicart-empty<% } %>" action="<%= config.action %>" target="<%= config.target %>">    <button type="button" class="minicart-closer" tabindex="0" aria-label="장바구니 닫기">&times;</button>    <ul>        <% for (var i= 0, idx = i + 1, len = items.length; i < len; i++, idx++) { %>        <li class="minicart-item">            <div class="minicart-details-name">                <span class="minicart-name"><%= items[i].get("item_name") %></span>                <ul class="minicart-attributes">                    <% if (items[i].get("item_number")) { %>                    <li>                        <%= items[i].get("item_number") %>                        <input type="hidden" name="item_number_<%= idx %>" value="<%= items[i].get("item_number") %>" />                    </li>                    <% } %>                    <% if (items[i].discount()) { %>                    <li>                        <%= config.strings.discount %> <%= items[i].discount(priceFormat) %>                        <input type="hidden" name="discount_amount_<%= idx %>" value="<%= items[i].discount() %>" />                    </li>                    <% } %>                    <% for (var options = items[i].options(), j = 0, len2 = options.length; j < len2; j++) { %>                        <li>                            <%= options[j].key %>: <%= options[j].value %>                            <input type="hidden" name="on<%= j %>_<%= idx %>" value="<%= options[j].key %>" />                            <input type="hidden" name="os<%= j %>_<%= idx %>" value="<%= options[j].value %>" />                        </li>                    <% } %>                </ul>            </div>            <div class="minicart-details-quantity">        <div class="minicart-quantity" data-minicart-idx="<%= i %>" name="quantity_<%= idx %>" ><%= items[i].get("quantity") %>개</div>     <!--   <input class="minicart-quantity" aria-label="수량" data-minicart-idx="<%= i %>" name="quantity_<%= idx %>" type="text" pattern="[0-9]*" value="<%= items[i].get("quantity") %>" autocomplete="off" />   -->         </div>            <div class="minicart-details-remove">                <button type="button" class="minicart-remove" aria-label="삭제" data-minicart-idx="<%= i %>">&times;</button>            </div>            <div class="minicart-details-price">                <span class="minicart-price"><%= items[i].total(priceFormat) %></span>            </div>            <input type="hidden" name="item_name_<%= idx %>" value="<%= items[i].get("item_name") %>" />            <input type="hidden" name="amount_<%= idx %>" value="<%= items[i].amount() %>" />            <input type="hidden" name="shipping_<%= idx %>" value="<%= items[i].get("shipping") %>" />            <input type="hidden" name="shipping2_<%= idx %>" value="<%= items[i].get("shipping2") %>" />        </li>        <% } %>    </ul>    <div class="minicart-footer">        <% if (hasItems) { %>            <div class="minicart-subtotal">                <%= config.strings.subtotal %> <%= cart.total(totalFormat) %>            </div>            <button class="minicart-submit" type="submit" data-minicart-alt="<%= config.strings.buttonAlt %>"><%- config.strings.button %></button>        <% } else { %>            <p class="minicart-empty-text"><%= config.strings.empty %></p>        <% } %>    </div>    <input type="hidden" name="cmd" value="_cart" />    <input type="hidden" name="upload" value="1" />    <% for (var key in settings) { %>        <input type="hidden" name="<%= key %>" value="<%= settings[key] %>" />    <% } %></form>',
            styles: '@keyframes pop-in {    0% { opacity: 0; transform: scale(0.1); }    60% { opacity: 1; transform: scale(1.2); }    100% { transform: scale(1); }}@-webkit-keyframes pop-in {    0% { opacity: 0; -webkit-transform: scale(0.1); }    60% { opacity: 1; -webkit-transform: scale(1.2); }    100% { -webkit-transform: scale(1); }}@-moz-keyframes pop-in {    0% { opacity: 0; -moz-transform: scale(0.1); }    60% { opacity: 1; -moz-transform: scale(1.2); }    100% { -moz-transform: scale(1); }}.minicart-showing #PPMiniCart {    display: block;    transform: translateZ(0);    -webkit-transform: translateZ(0);    -moz-transform: translateZ(0);    animation: pop-in 0.25s;    -webkit-animation: pop-in 0.25s;    -moz-animation: pop-in 0.25s;}#PPMiniCart {    display: none;    position: fixed; width:100%;    left: 50%;    top: 75px; background: rgba(255, 255, 255, 0.7); }#PPMiniCart form {    position: relative;    width: 400px;    max-height: 400px;    margin: 162px 27px 100%;    padding: 10px 10px 40px;    background: #fbfbfb;    border: 1px solid #d7d7d7;    border-radius: 4px;    box-shadow: 2px 2px 10px rgba(0, 0, 0, 0.5);    font: 15px/normal arial, helvetica;    color: #333;}#PPMiniCart form.minicart-empty {    padding-bottom: 10px;    font-size: 16px;    font-weight: bold;}#PPMiniCart ul {    clear: both;    float: left;    width: 380px;    margin: 5px 0 20px;    padding: 10px;    list-style-type: none;    background: #fff;    border: 1px solid #ccc;    border-radius: 4px;    box-shadow: 1px 1px 3px rgba(0, 0, 0, 0.2);}#PPMiniCart .minicart-empty ul {    display: none;}#PPMiniCart .minicart-closer {    float: right;    margin: -8px -10px 0;    padding: 10px;    background: 0;    border: 0;    font-size: 24px;    cursor: pointer;    font-weight: bold;}#PPMiniCart .minicart-item {    clear: left;    padding: 6px 0;    min-height: 25px; opacity: 0;}#PPMiniCart .minicart-item + .minicart-item {    border-top: 1px solid #f2f2f2;}#PPMiniCart .minicart-item a {    color: #333;    text-decoration: none;}#PPMiniCart .minicart-details-name {    float: left;    width: 49%;}#PPMiniCart .minicart-details-quantity {    float: left;    width: 13%;}#PPMiniCart .minicart-details-remove {    float: left;    width: 9%;}#PPMiniCart .minicart-details-price {    float: left;    width: 27%;    text-align: right;}#PPMiniCart .minicart-attributes { display:none;    margin: 0;    padding: 0;    background: transparent;    border: 0;    border-radius: 0;    box-shadow: none;    color: #999;    font-size: 12px;    line-height: 22px;}#PPMiniCart .minicart-attributes li {    display: inline;}#PPMiniCart .minicart-attributes li:after {    content: ",";}#PPMiniCart .minicart-attributes li:last-child:after {    content: "";}#PPMiniCart .minicart-quantity {    width: 100%;    height: 18px;    padding: 1px 4px;  /*  border: 1px solid #ccc;    border-radius: 4px;    box-shadow: inset 0 1px 1px rgba(0, 0, 0, 0.075); */   font-size: 16px;    text-align: center;    transition: border linear 0.2s, box-shadow linear 0.2s;    -webkit-transition: border linear 0.2s, box-shadow linear 0.2s;    -moz-transition: border linear 0.2s, box-shadow linear 0.2s;}#PPMiniCart .minicart-quantity:hover {    border-color: #0078C1;}#PPMiniCart .minicart-quantity:focus {    border-color: #0078C1;    outline: 0;    box-shadow: inset 0 1px 1px rgba(0, 0, 0, 0.075), 0 0 3px rgba(0, 120, 193, 0.4);}#PPMiniCart .minicart-remove {    width: 21px;    height: 19px;   /* margin: 2px 0 0; */   padding: 0;    background: #b7b7b7;    border: 1px solid #a3a3a3;    border-radius: 3px;    color: #fff;    font-size: 13px;    opacity: 0.70;    cursor: pointer;}#PPMiniCart .minicart-remove:hover {    opacity: 1;}#PPMiniCart .minicart-footer {    clear: left;}#PPMiniCart .minicart-subtotal {    position: absolute;    bottom: 17px;    padding-left: 6px;    left: 10px;    font-size: 16px;    font-weight: bold;}#PPMiniCart .minicart-submit {    position: absolute;    bottom: 10px;    right: 10px;    min-width: 153px;    height: 33px;    margin-right: 6px;    padding: 0 9px;    border: 1px solid #ffc727;    border-radius: 5px;    color: #000;    text-shadow: 1px 1px 1px #fff6e9;    cursor: pointer;    background: #ffaa00;    background: url(data:image/svg+xml;base64,);    background: -moz-linear-gradient(top, #fff6e9 0%, #ffaa00 100%);    background: -webkit-gradient(linear, left top, left bottom, color-stop(0%,#fff6e9), color-stop(100%,#ffaa00));    background: -webkit-linear-gradient(top, #fff6e9 0%,#ffaa00 100%);    background: -o-linear-gradient(top, #fff6e9 0%,#ffaa00 100%);    background: -ms-linear-gradient(top, #fff6e9 0%,#ffaa00 100%);    background: linear-gradient(to bottom, #fff6e9 0%,#ffaa00 100%);}#PPMiniCart .minicart-submit img {    vertical-align: middle;    padding: 4px 0 0 2px;}',
            strings: {
                button: '',
                subtotal: "총액:",
                discount: "Discount:",
    ...
    12: [function(a, b) {
        ...
        ,
        "undefined" == typeof window ? b.exports = i : (window.paypal || (window.paypal = {}),
    ...
    13: [function(a, b) {
        ...
        c.prototype.options = function() {
            ...
            if (!this._options) {
                ...
                e = 0; b = this.get("on" + e); ) {
                    for (c = this.get("os" + e),
                    d = 0,
                    f = 0; "undefined" != typeof this.get("option_select" + f); ) {
        ...
        c.prototype.discount = function(a) {
            var b, c, e, f, h, i;
            return this._discount || (h = 0,
            e = parseInt(this.get("discount_num"), 10) || 0,
            f = Math.max(e, this.get("quantity") - 1),
            void 0 !== this.get("discount_amount") ? (b = g.amount(this.get("discount_amount")),
            h += b,
            h += g.amount(this.get("discount_amount2") || b) * f) : void 0 !== this.get("discount_rate") && (c = g.amount(this.get("discount_rate")),
            i = this.amount(),
            h += c * i / 100,
        ...
        c.prototype.total = function(a) {
            var b;
            return this._total || (b = this.get("quantity") * this.amount(),
            b -= this.discount(),
    ...
    14: [function(a, b) {
        ...
        b.exports.add = function(a, b) {
            var c;
            return a ? void (a && a.classList && a.classList.add ? a.classList.add(b) : (c = new RegExp("\\b" + b + "\\b"),
            c.test(a.className) || (a.className += " " + b))) : !1
        }
        ,
        b.exports.remove = function(a, b) {
            var c;
            return a ? void (a.classList && a.classList.add ? a.classList.remove(b) : (c = new RegExp("\\b" + b + "\\b"),
            c.test(a.className) && (a.className = a.className.replace(c, "")))) : !1
        }
        ,
    ...
    15: [function(a, b) {
        ...
        b.exports = function(a, b) {
            var d = b && b.currency || "KRW"
              ...
              , j = a;
            return b && b.format && (j = f + j.toFixed(h) + g),
            i && (j += " " + d),
            j
        }
    }
    ...
    17: [function(a, b) {
        ...
        var c = b.exports = {
            parse: function(a) {
                ...
                f = g.length; f > e; e++)
                    b = g[e],
                    (d = c.getInputValue(b)) && (h[b.name] = d);
                return h
            },
    ...
    18: [function(a, b) {
        ...
        b.exports = function c(a, b) {
            ...
            for (var e in b)
                d = b[e],
                d && d.constructor === Object && a[e] ? c(a[e] || {}, d) : a[e] = d;
            return a
        }
    }
    , {}],
    ...
    20: [function(a, b) {
        ...
        d.load = function() {
            if ("object" == typeof window && window.localStorage) {
                ...
                c && c.expires && (a = new Date,
                b = new Date(c.expires),
                a > b) ? void this.destroy() : c && c.value
            }
        }
        ,
        d.save = function(a) {
    ...
    22: [function(a, b) {
        ...
        function c(a) {
            ...
            e.add(document, "touchstart", i.click, this),
            e.add(document, "click", i.click, this),
            ...
            document.addEventListener("focus", i.focus, true)
        }
        var d = a("./config")
          , e = a("./util/events")
          , f = a("./util/template")
        ...
        c.prototype.show = function() {
            this.isShowing || (h.add(document.body, j.SHOWING_CLASS),
            this.lastFocusedElem = document.activeElement,
            document.getElementById("PPMiniCart").focus(),
            document.getElementById("MainBody").setAttribute("aria-hidden", "true"),
            this.isShowing = !0)
        }
        ,
        c.prototype.hide = function() {
            this.isShowing && (h.remove(document.body, j.SHOWING_CLASS),
            this.lastFocusedElem ? this.lastFocusedElem.focus() : '',
            this.lastFocusedElem = null,
            document.getElementById("MainBody").setAttribute("aria-hidden", "false"),
            this.isShowing = !1)
        }
        ,
        ...
        c.prototype.bind = function(a) {
            var b = this;
            return j.COMMANDS[a.cmd.value] ? a.hasMinicart ? !1 : (a.hasMinicart = !0,
            a.display ? e.add(a, "submit", function(a) {
                a.preventDefault(),
                b.show()
            }) : e.add(a, "submit", function(c) {
                c.preventDefault(c),
                b.model.cart.add(g.parse(a))
            }),
            !0) : !1
        }
        ,
    ...
    23: [function(a, b) {
        ...
        b.exports = c = {
            focus: function(a) {
                var b = a.target;
                if (document.body.classList.contains("minicart-showing")) {
                    var cartElem = document.getElementById("PPMiniCart");
                    for (; 1 === b.nodeType; ) {
                            if (b === cartElem)
                                return;
                            b = b.parentNode
            ...
            keyup: function(a) {
                ...
                e.className === d.QUANTITY_CLASS && (b = setTimeout(function() {
                    ...
                    f && ((g > 0 && f.get("quantity") != g) ? f.set("quantity", g) : 0 === g && b.remove(a))
                }, d.KEYUP_TIMEOUT))
            },
            readystatechange: function() {
                if (/interactive|complete/.test(document.readyState)) {
                    var a, b, f, g;
                    for (a = document.getElementsByTagName("form"),
                    f = 0,
                    g = a.length; g > f; f++)
                        b = a[f],
                        b.cmd && d.COMMANDS[b.cmd.value] && this.bind(b);
                    this.redraw(),
